...
class D1S_Input(Input):
    ...
    def get_xs(mat_nuc, mt: int, lib: str, lib_manager: LibManager, 
        ...
        if material:
            energies, xs, fractions = Input.get_material_xs(mat_nuc, mt, lib, lib_manager=lib_manager)
            return Input._build_material_xs(energies, xs, fractions)
        else:
            data_t = Input.get_zaid_xstable(str(mat_nuc)+'.'+lib, lib, lib_manager)
            return Input.get_reaction_xs(data_t, mt)
    ...
    def get_material_xs(mat, mt: int, lib : str, lib_manager: LibManager):
        ...
        for submat in mat.submaterials:
            for zaid in submat.zaidList:
                data_t = Input.get_zaid_xstable(zaid.name, lib, lib_manager)
                
                x, y = Input.get_reaction_xs(data_t, mt)
    ...
    def partial_energy_matrix(E_g, E_n):
        ...
        if (E_g[:-1] >= E_g[1:]).all() and (E_n[:-1] >= E_n[1:]).all():
            ...
            assert E_g[0] <= E_n[0]
            assert E_n[-1] <= E_g[-1]
            pem = Input.partial_energy_matrix_mono(E_g, E_n, -1)
    ...
    def phi_g(E_g: np.ndarray, E_n: np.ndarray, phi_n: np.ndarray):
        ...
        """
        pem = Input.partial_energy_matrix(E_g, E_n)
    ...
    def group_collapse(sigma_n: np.ndarray, phi_n: np.ndarray, 
        ...
        Parameters
        ----------
        sigma_n : numpy 1d array
            A high-fidelity cross-section.
        phi_n : numpy 1d array
            The high-fidelity flux [n/cm^2/s] to collapse the cross-section 
            over (length N).  
        phi_g : array-like of floats, optional
            The low-fidelity flux [n/cm^2/s] to collapse the cross-section 
        ...
        -------
        sigma_g : ndarray
            An array of the collapsed cross-section.
        """
    ...
    def ninespace(start, stop, num=50, endpoint=True):
        ...
        samples = 1.0 - np.logspace(log_start, log_stop, num, endpoint)
        return samples
    ...
    def pointwise_linear_collapse(x_g, x, y):
        ...
        """
        return Input.pointwise_collapse(x_g, x, y)
    ...
    def collapse_xs(x_g, x, y):
        ...
        if idx != 0:
            ...
            xs = Input.pointwise_linear_collapse(new_x_g, x, y)
            xs = np.concatenate((np.zeros(len(x_g)-len(new_x_g)), xs))
        else:
            xs = Input.pointwise_linear_collapse(x_g, x, y)
